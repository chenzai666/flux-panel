...
public class ForwardServiceImpl extends ServiceImpl<ForwardMapper, Forward> implements ForwardService {
    ...
    public void syncAllToGost() {
        List<Forward> forwards = this.list(new QueryWrapper<Forward>().eq("status", FORWARD_STATUS_ACTIVE));
        ...
        for (Forward forward : forwards) {
            try {
                ...
                UserTunnel userTunnel = getUserTunnel(forward.getUserId(), tunnel.getId().intValue());
                NodeInfo nodeInfo = getRequiredNodes(tunnel);
                if (nodeInfo.isHasError()) { skip++; continue; }
                Integer limiter = userTunnel != null ? userTunnel.getSpeedId() : null;
                R result = createGostServices(forward, tunnel, limiter, nodeInfo, userTunnel);
                if (result.getCode() == 0) ok++; else skip++;
            } catch (Exception e) {
                log.warn("同步转发 {} 失败: {}", forward.getId(), e.getMessage());
                skip++;
            }
        }
