...
export default function UserPage() {
  ...
  const visibleSelectedUserIds = users
    .map((u) => u.id)
  ...
  const handleSelectAll = () => {
    if (visibleSelectedUserIds.length === users.length && users.length > 0) {
      setSelectedUserIds(new Set());
    } else {
  ...
  const handleConfirmBatchDelete = async () => {
    if (visibleSelectedUserIds.length === 0) return;
    ...
    try {
      ...
      for (const userId of visibleSelectedUserIds) {
      ...
      if (failCount === 0) {
        ...
        toast.error(`成功删除 ${successCount} 个用户，失败 ${failCount} 个`);
      }
      ...
      <div className="flex flex-col gap-4 mb-4 sm:mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
          <div className="flex items-center gap-3 flex-1 max-w-md">
            <Input
              ...
              classNames={{
                ...
                inputWrapper:
                  "bg-[#f0ece6] dark:bg-[#2d2824] border-2 border-[#e5e0d8] dark:border-[#2d2824] hover:border-[#c96442] focus-within:border-[#c96442] data-[hover=true]:border-[#c96442]",
              }}
              placeholder="搜索用户名"
              startContent={
                <SearchIcon className="w-4 h-4 text-[#9b9590] dark:text-[#5d5854]" />
              }
              value={searchKeyword}
          ...
          <div className="flex items-center gap-3">
            {users.length > 0 && (
              <Checkbox
                isSelected={visibleSelectedUserIds.length === users.length && users.length > 0}
                isIndeterminate={visibleSelectedUserIds.length > 0 && visibleSelectedUserIds.length < users.length}
            ...
            {visibleSelectedUserIds.length > 0 && (
              <>
                <span className="text-sm text-[#6b6560] dark:text-[#8a8480]">
                  已选择 {visibleSelectedUserIds.length} 项
                </span>
                <Button
                  color="danger"
                  size="sm"
